...
void loop() {
  ...
  data.Lx = map(analogRead(PA0), 0, 1023, 1000, 2000); // Convert the analog read value from 0 to 1023 into a BYTE value from 0 to 255
  data.Ly = map(analogRead(PA1), 0, 1023, 1000, 2000);
  data.Rx = map(analogRead(PA3), 0, 1023, 2000, 1000);
  data.Ry = map(analogRead(PA2), 0, 1023, 2000, 1000);
